#!/usr/bin/env python3
"""
repomap.py — Tree-sitter + PageRank codebase indexer (Stellar Powers).
...
Usage:
    ...
    python repomap.py --install-deps
...
def index_codebase(root_path, force=False):
        ...
        try:
            ...
            if cached.get("version") == 2:
                ...
                if not stale:
                    ...
                    print(f"[code-indexer] Loaded cached index: {len(idx.files)} files, {len(idx.symbols)} symbols", file=sys.stderr)
                    return idx
        except (json.JSONDecodeError, KeyError):
            pass

    # Fresh index
    print(f"[code-indexer] Indexing {root}...", file=sys.stderr)
    idx = RepoIndex(root_path=str(root))
    ...
    print(f"[code-indexer] Indexed {len(idx.files)} files, {len(idx.symbols)} symbols", file=sys.stderr)
    return idx
...
def install_deps():
    """Install tree-sitter, networkx, and common language grammars."""
    import subprocess

    core_packages = ["tree-sitter", "networkx"]
    grammar_packages = [
        "tree-sitter-python",
    ...
    all_packages = core_packages + grammar_packages

    print(f"[code-indexer] Installing {len(all_packages)} packages...", file=sys.stderr)

    for pkg in all_packages:
        print(f"  Installing {pkg}...", file=sys.stderr, end=" ")
        ...
        if result.returncode == 0:
            ...
            print(f"FAILED ({result.stderr.strip()[:80]})", file=sys.stderr)

    print("[code-indexer] Done. Run again without --install-deps to index a project.", file=sys.stderr)
...
def main():
    # Handle --install-deps before argparse (no positional 'path' needed)
    if "--install-deps" in sys.argv:
        install_deps()
        sys.exit(0)

    parser = argparse.ArgumentParser(
        description="Tree-sitter + PageRank codebase indexer (Stellar Powers)",
