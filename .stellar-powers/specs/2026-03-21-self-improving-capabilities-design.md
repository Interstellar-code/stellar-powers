...
All `prompt_preview`, `response_preview`, `correction`, and `completion_feedback` fields pass through a redaction filter before being written to workflow.jsonl or metrics packages. The filter strips:
- Patterns matching API keys, tokens, passwords (regex: common key formats, `sk-`, `ghp_`, `Bearer`, etc.)
- Email addresses, phone numbers
- File paths containing `/Users/` or `~` are replaced with relative paths

The stellar-powers repo is **private**. GitHub Issues filed by send-feedback inherit this. If the repo becomes public, the redaction filter must be audited first.

### Kill Switch

Feedback capture can be disabled via `.stellar-powers/config.json`:

```json
{"feedback_enabled": false}
```

When `false`: hooks skip all workflow.jsonl writes, skills skip step logging and correction capture, send-feedback reports "feedback is disabled." The workflow lifecycle (`.active-workflow`, completion checkpoint) continues to function — only the data capture layer is disabled.

Default: `true`. If absent, treated as `true`.

### Concurrency

Claude Code typically runs one session per working directory. For the rare case of concurrent sessions:
- `workflow.jsonl`: JSONL append-only writes are atomic at the OS level for single-line writes (POSIX guarantees for `O_APPEND` on lines < `PIPE_BUF`). Each event is a single line. No locking needed.
- `.active-workflow`: Skills write atomically via write-to-temp-then-rename pattern (`write to .active-workflow.tmp`, then `mv .active-workflow.tmp .active-workflow`). Last writer wins, which is acceptable — concurrent workflows in the same repo is an edge case that warrants user discipline, not complex locking.

### `gh` CLI Availability

The send-feedback skill checks `gh auth status` as its first step. If `gh` is not installed or not authenticated, it fails immediately with a clear message before touching any metrics files. Metrics packages are never deleted unless the corresponding GitHub issue is confirmed created.
...
```
This is also the mechanism for capturing AI responses for the metrics package. The `Stop` hook fires after every agent turn and provides `last_assistant_message`. The `response_preview` (first 300 chars) is stored in workflow.jsonl. At packaging time, the packager reads these `turn_completed` events — no need to access `transcript_path` at packaging time.
...
**Hook behavior when no workflow is active:** If `.active-workflow` does not exist, hooks silently skip — no logging, no warnings. This prevents noise during non-stellar-powers Claude Code sessions. The hooks only produce workflow.jsonl events when a stellar-powers skill has been invoked and set up the `.active-workflow` file.

**Workflow ID generation:** Generated via `uuidgen` (macOS/Linux) or `python3 -c "import uuid; print(uuid.uuid4())"` as fallback. Full UUID format (e.g., `7DD9E572-D266-49E4-A454-4DE6A75E0647`). Generated by the first skill in a workflow chain (typically brainstorming). Uniqueness is guaranteed by UUID v4 randomness — collision probability is negligible.
...
If user parks a workflow, the `.active-workflow` file is renamed to `.active-workflow.held.<workflow_id>` (e.g., `.active-workflow.held.7DD9E572`). This allows multiple workflows to be on hold simultaneously. The invocation gate scans for `.active-workflow.held.*` files and mentions them briefly ("Note: 2 workflows on hold: feature-porting, sdd-batching") but doesn't block.

To resume a held workflow, the user selects it at the invocation gate and the held file is renamed back to `.active-workflow`.

### Stale and Corrupted State Recovery

- **Stale workflows:** If `.active-workflow` references a workflow started more than 30 days ago, the invocation gate flags it: "This workflow has been inactive for N days. Complete, abandon, or hold?" No automatic timeout — the user decides.
- **Corrupted `.active-workflow`:** If the file exists but is not valid JSON, the invocation gate deletes it with a warning: "Corrupted workflow state detected and cleared. Starting fresh."
- **Orphaned `.active-workflow` after crash:** The invocation gate detects this when the workflow_id in `.active-workflow` has a `workflow_completed` or `workflow_abandoned` event in workflow.jsonl — the file is stale and gets cleaned up automatically.
...
3. For each package:
   ...
   e. Rename metrics file to `.sent` suffix (e.g., `...7DD9E572.json.sent`)
   f. If issue creation fails, skip this package and continue to next — report failures at end
4. After all packages processed, delete `.sent` files
5. Log `feedback_sent` event to workflow.jsonl
6. Report: "N issues created on stellar-powers: [URLs]. M failed (files retained)."

**Idempotency:** The rename-to-`.sent`-then-delete pattern prevents duplicates. If the skill crashes between rename and delete, `.sent` files are ignored on the next run (the scan only looks for `.json` files). If issue creation fails, the `.json` file remains — the next run will retry it.

**Deduplication guard:** Before creating an issue, the skill searches for existing issues with the same workflow_id in the title: `gh search issues --repo stellar-powers "workflow_id" --state open`. If found, skip and report "already submitted."

### Why GitHub Issues (not a repo file)

Considered alternative: append-only JSON file committed directly to the stellar-powers repo. Trade-offs:
- **Issues:** human-readable, searchable, labelable, closeable, no merge conflicts. Fragile: can be accidentally closed/deleted, rate-limited.
- **Repo file:** durable, version-controlled, no rate limits. But: merge conflicts if multiple repos push feedback concurrently, clutters git history.

Decision: **Issues for Phase 1.** They're easier to work with, human-readable for manual review, and the analysis agent can read them via `gh` CLI. If issues prove fragile at scale, migrate to repo-committed files in Phase 2.

### Prerequisites

- `gh` CLI authenticated with access to stellar-powers repo (checked at skill start via `gh auth status`)
...
Shared logic is embedded as **inline bash snippets within each skill's SKILL.md** — not separate shell scripts. Skills are markdown files containing instructions for Claude. Claude executes bash via the Bash tool. The "shared utilities" are documented code blocks that each skill includes at the appropriate points:

- **Workflow gate snippet:** ~10 lines of bash to read/create `.active-workflow`
- **Step logging snippet:** ~3 lines to append `step_started`/`step_completed` to workflow.jsonl
- **Metrics packaging snippet:** ~20 lines to build the JSON package and prune

These snippets are maintained in a reference file `skills/_shared/snippets.md` (read-only reference, not executed directly). Each skill copies the relevant snippet into its own SKILL.md. When a snippet is updated, all skills that use it must be updated — this is a maintenance trade-off accepted for simplicity over a shared script execution mechanism.

```
skills/
├── _shared/
│   └── snippets.md           # reference snippets for lifecycle, step logging, packaging
├── brainstorming/
├── writing-plans/
├── send-feedback/            # NEW
...
...
**Partial approvals:** Responses like "mostly good, but also add X" are logged as corrections with category `partial_approval`. The correction text captures the "but also add X" part. Clarifying questions at gates ("what do you mean by X?") are not corrections — the skill should answer and re-ask the gate question. The distinction: a correction changes the output, a clarification seeks understanding before approving.
...
**Minimum sample size:** Do not declare improvement or regression with fewer than 10 workflow completions per version per skill. Below this threshold, noise dominates signal. Report raw numbers without conclusions.

### Schema Evolution

The metrics package includes `package_version` (currently `"1.0"`). If the schema changes:
- The send-feedback skill handles all known package versions — it reads `package_version` and adapts its issue formatting accordingly
- Old packages in `.stellar-powers/metrics/` are sent as-is with their original schema — no migration needed
- The analysis agent (Phase 2) must also handle multiple schema versions when reading GitHub issues

## Phase Graduation Criteria

**Phase 1 → Phase 2 trigger:** When all of the following are met:
- At least 20 workflow completions across 3+ repos have been submitted via send-feedback
- At least 3 skills have 10+ data points each
- Manual review of GitHub issues reveals actionable patterns (not just noise)

**Phase 2 → Phase 3 trigger:** When the analysis agent has successfully proposed and applied 5+ skill improvements that show measurable improvement in subsequent feedback.
